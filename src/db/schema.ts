...
// ----------------------------------------- explored_paths (the exploration memory)
// Every direction a user samples in a mentor call (via a card dive, or an
// exploration-stance insight) becomes a saved BRANCH. Powers the "you've explored
// N" comparison view + the journey-evolution log + the pricing gate: exploring is
// free (the funnel), `committed_at` marks the paid step-up (intro + apply kit).

export const exploredPaths = pgTable("explored_paths", {
  id: uuid("id").primaryKey().defaultRandom(),
  profileId: uuid("profile_id")
    .notNull()
    .references(() => profiles.id, { onDelete: "cascade" }),
  label: text("label").notNull(), // the role title = the branch label
  company: text("company"),
  kind: text("kind"), // STRONG FIT / A DIFFERENT PATH / A PIVOT
  // arc / comp / transfer difficulty / first move / circle person — the mentor's
  // dossier, enriched over time. Starts from what the card carried.
  summary: jsonb("summary").$type<Record<string, unknown>>().default({}),
  source: text("source").default("card_dive"), // card_dive | exploration_insight
  visitCount: integer("visit_count").default(1).notNull(),
  committedAt: timestamp("committed_at", { withTimezone: true }), // the paid step-up moment
  createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
